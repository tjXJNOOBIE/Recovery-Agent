...
test('runsSimulatedFleetThroughRecoveryAndEscalationEndToEnd', async () => {
  ...
  assert.deepEqual(sweep.map((entry) => entry.recovery?.status).sort(), ['escalated', 'recovered'])
  ...
  assert.deepEqual(watches.map((watch) => watch.lastStatus).sort(), ['escalated', 'recovered'])
