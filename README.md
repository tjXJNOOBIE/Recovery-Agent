...
MCP host
  -> Recovery MCP server
      -> deterministic control runtime
          ...
          -> Strands investigation only after deterministic recovery is exhausted
```
...
- `payments`: remains failed after its restart budget, then receives a simulated investigation and human escalation.
...
The deterministic control runtime invokes Strands only after a service cannot be restored within its configured restart budget. The investigator receives incident evidence and returns diagnosis text. It does not execute recovery actions.
