...
export interface RecoveryNodeConfig { readonly nodeId: string; readonly listenHost: string; readonly listenPort: number; readonly tokenEnvironmentVariable: string; readonly services: readonly RecoveryNodeServiceConfig[]; readonly certificates: readonly CertificateDefinition[] }

export class RecoveryNodeConfigReader {
  public read(path: string): RecoveryNodeConfig {
    const parsed = JSON.parse(readFileSync(path, 'utf8')) as unknown; const record = this.requireRecord(parsed, 'node config'); const servicesValue = record['services']
    if (!Array.isArray(servicesValue) || servicesValue.length === 0) throw new Error('node config services must be a non-empty array')
    return {
      nodeId: this.requireString(record, 'nodeId'), listenHost: this.optionalString(record, 'listenHost') ?? '127.0.0.1', listenPort: this.optionalNumber(record, 'listenPort') ?? 7843,
      tokenEnvironmentVariable: this.requireString(record, 'tokenEnvironmentVariable'), certificates: this.readCertificates(record),
      services: servicesValue.map((value, index) => { const service = this.requireRecord(value, `services[${index}]`); const deploymentMarkerFile = this.optionalString(service, 'deploymentMarkerFile'); return { id: this.requireString(service, 'id'), unit: this.requireString(service, 'unit'), healthChecks: this.readHealthChecks(service, `services[${index}].healthChecks`), ...(deploymentMarkerFile === undefined ? {} : { deploymentMarkerFile }) } }),
    }
  }
  ...
  private optionalNumber(record: Readonly<Record<string, unknown>>, key: string): number | undefined { const value = record[key]; if (value === undefined) return undefined; if (typeof value !== 'number' || !Number.isInteger(value) || value < 0 || value > 65_535) throw new Error(`${key} must be a valid TCP port`); return value }
