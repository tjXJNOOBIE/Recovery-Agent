...
async function main(): Promise<void> {
  ...
  if (command === 'node') {
    ...
    await new Promise<void>((resolve) => { const stop = (): void => { void server.close().finally(resolve) }; process.once('SIGINT', stop); process.once('SIGTERM', stop) })
    return
  }
  if (command === 'mcp') {
    ...
    const watches = new RecoveryWatchCoordinator(serviceWatches, nodeWatches, 1_000, readinessWatch, certificateWatch)
    ...
    await new Promise<void>((resolve, reject) => {
      let closing = false
      const stop = (): void => { if (closing) return; closing = true; void mcpServer.close().then(() => watches.close()).then(() => approvalServer?.close()).then(() => control.close()).then(resolve, reject) }
