...
export class RecoveryMcpServer {
  ...
  public serve(): void {
    if (this.handle !== undefined) {
      throw new Error('Recovery MCP server is already serving')
    }
  ...
  private createServer(): McpServer {
    ...
    server.registerTool(
      'fleet_status',
      {
        description: 'Inspect current node and service health.',
    ...
    server.registerTool(
      'service_inspect',
      {
        ...
        inputSchema: nodeServiceInputSchema,
        annotations: { readOnlyHint: true },
      },
      async ({ nodeId, serviceId }) => this.toolResult(
        await this.toolRouter.callTool('service_inspect', { nodeId, serviceId }),
      ),
    )
    ...
    server.registerTool(
      'service_recover',
      {
        description: 'Run bounded policy-controlled recovery for one configured service.',
        inputSchema: nodeServiceInputSchema,
        annotations: { readOnlyHint: false, destructiveHint: false },
      },
      async ({ nodeId, serviceId }) => this.toolResult(
        await this.toolRouter.callTool('service_recover', { nodeId, serviceId }),
      ),
    )
    ...
    server.registerTool(
      'health_sweep',
      {
        description: 'Check configured services and recover unhealthy services within policy.',
        inputSchema: emptyInputSchema,
        annotations: { readOnlyHint: false, destructiveHint: false },
      },
    ...
    server.registerTool(
      'incident_inspect',
      {
        description: 'Inspect one recovery incident and its timeline.',
        inputSchema: incidentInputSchema,
        annotations: { readOnlyHint: true },
      },
      async ({ incidentId }) => this.toolResult(
        await this.toolRouter.callTool('incident_inspect', { incidentId }),
      ),
    )
