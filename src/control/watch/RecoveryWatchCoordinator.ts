...
export class RecoveryWatchCoordinator implements RecoveryWatchSurface {
  ...
  public constructor(serviceWatchService: RecoveryWatchService, nodeWatchService: RecoveryNodeWatchService, pulseMs = 1_000, readinessWatchService?: RecoveryReadinessWatchService, certificateWatchService?: RecoveryCertificateWatchService, deploymentWatchService?: RecoveryDeploymentWatchService) {
    ...
    this.serviceWatchService = serviceWatchService; this.nodeWatchService = nodeWatchService; this.readinessWatchService = readinessWatchService; this.certificateWatchService = certificateWatchService; this.deploymentWatchService = deploymentWatchService; this.pulseMs = pulseMs
  }
  ...
  public async runAllNow(nowMs = Date.now()) {
    ...
    const deployments = this.deploymentWatchService === undefined ? undefined : await this.deploymentWatchService.runNow(nowMs)
  ...
  private runFromTimer(): void { if (this.inFlight !== undefined) return; const execution = this.runDueWatches(); this.inFlight = execution; void execution.then(() => this.clearInFlight(execution), () => this.clearInFlight(execution)) }
  ...
  private async runDueWatches(nowMs = Date.now()): Promise<unknown> {
    ...
    const deployments = this.deploymentWatchService === undefined ? undefined : await this.deploymentWatchService.runDueWatch(nowMs)
